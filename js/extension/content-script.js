...
appendFragment([resourceLibrary]).then(() => {
  ...
  chrome.storage.sync.get('prefs', result => {
    ...
    .then(() => {
      // TODO: set default values if these are undefined
      ...
      return new Promise(async (resolve) => {
        ...
        let target = userPreferences,
            currentFilterState = getCurrentFilterState(),
            userCurrency = prefs.userCurrency,
            newPrefs;

        // Create new userPreferences object from old and current preferences
        newPrefs = Object.assign(target, oldPrefs, { currentFilterState }, { userCurrency });
